<template>
    <div class="chat-main-layout">
        <div v-if="!selectedChat" class="chat-list">
            ...
            <div v-else-if="conversations.length > 0">
                <div class="header-section">
                    <h4>Mensajes del Marketplace</h4>
                    <p class="text-muted" style="color: white">
                        Conversaciones sobre tus productos
                    </p>
                </div>
                ...
                <div v-if="filteredConversations.length > 0" class="conversations-list">
                    <div 
                        ...
                        :key="`${chat.marketplace_post_id}-${chat.user_id}`" 
                        class="chat-item"
                        :class="{ active: selectedChat && selectedChat.marketplace_post_id === chat.marketplace_post_id && selectedChat.user_id === chat.user_id }" 
                    ...
                    >
                        ...
                        <div class="chat-content">
                            ...
                            <div class="post-section">
                                ...
                                <div class="post-text">
                                    <span class="post-tag">{{ getCategoryLabel(chat.post_category) }}</span>
                                    <p class="post-desc">{{ truncateMessage(chat.post_title, 50) }}</p>
                                </div>
                            </div>
            ...
            <div v-else class="empty-state">
                ...
                <span class="empty-subtitle">Tus mensajes del marketplace aparecerán aquí</span>
            </div>
        </div>
        ...
        <div v-else class="chat-content">
                ...
                <div class="header-info">
                    ...
                    <div class="header-text">
                        ...
                        <p class="header-subtitle">
                            <i class="bi bi-tag"></i>
                            {{ truncateMessage(selectedChat.post_title, 35) }}
                        </p>
                    </div>
                </div>
                <img
            ...
            <MarketPostChatView 
                :post-id="selectedChat.marketplace_post_id"
                :recipient-id="selectedChat.user_id" 
                :current-user-id="currentUserId"
                :chat-data="selectedChat"
                :key="`${selectedChat.marketplace_post_id}-${selectedChat.user_id}`" 
            />
        </div>
    </div>
</template>
...
import MarketPostChatView from './MarketPostChatView.vue';
...
async function fetchChats() {
    loading.value = true;
    try {
        const response = await api.get(`${API_BASE_URL}/api/marketplace/messages/chats`);
        conversations.value = response.data;
    } catch (error) {
        console.error('Error fetching marketplace chats:', error);
    } finally {
        loading.value = false;
    }
}
...
const filteredConversations = computed(() => {
    ...
    return conversations.value.filter(chat =>
        ...
        chat.post_title?.toLowerCase().includes(query)
    );
});
...
function listenForNewMessages() {
    ...
    window.Echo.private(`user.${currentUserId.value}`).listen('.marketplace.message.received', (event) => {
        console.log('New marketplace message received:', event);
        
        const conversationIndex = conversations.value.findIndex(
            c => c.marketplace_post_id === event.marketplacePostId && c.user_id === event.fromUserId
        );
        ...
        if (conversationIndex !== -1) {
            ...
            if (!selectedChat.value || 
                selectedChat.value.marketplace_post_id !== event.marketplacePostId || 
...
<style scoped>
/* Same styles as PostMessages.vue */
.chat-main-layout {
    display: flex;
