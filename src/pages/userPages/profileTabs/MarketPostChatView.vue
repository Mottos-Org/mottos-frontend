...
async function fetchMessages() {
    ...
    try {
        const response = await api.get(
            `${API_BASE_URL}/api/marketplace/${props.postId}/messages/${props.recipientId}`
        );
...
async function sendMessage() {
    ...
    const tempMessage = {
        ...
        marketplace_post_id: props.postId,
    ...
    try {
        await api.post(`${API_BASE_URL}/api/marketplace/${props.postId}/messages`, {
...
async function markAsRead() {
    try {
        await api.put(`${API_BASE_URL}/api/marketplace/${props.postId}/messages/read`, {
...
function listenForMessages() {
    ...
    window.Echo.private(`user.${props.currentUserId}`).listen('.marketplace.message.received', (event) => {
        // Only add message if it's for this specific conversation
        if (event.marketplacePostId === props.postId && event.fromUserId === props.recipientId) {
            const formattedMessage = {
                id: Date.now(),
                content: event.message,
                sender_id: event.fromUserId,
                receiver_id: event.toUserId,
                marketplace_post_id: event.marketplacePostId,
