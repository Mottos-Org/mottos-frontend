    ...
    <div class="admin-post-detail">
        <div class="header-section">
            ...
            <p class="page-subtitle">
                Revisa los detalles del producto y los comentarios de moderación antes de tomar una decisión
            </p>
        </div>
        ...
        <div v-else class="post-section">
            <MarketplaceCard v-if="postData" :key="postData.publicacion_marketplace_id" :item="postData" />
                ...
                <ul v-else class="comments-list">
                    <li v-for="comment in comments" :key="comment.comment_id" class="comment-item">
                        <div class="comment-avatar">
                            <img
                                v-if="comment.admin?.profile?.profile_picture || comment.admin_profile_picture_url"
                                :src="comment.admin?.profile?.profile_picture ? comment.admin.profile.profile_picture : comment.admin_profile_picture_url"
            ...
            <transition name="fade-modal">
                <div v-if="showApprovalModal" class="modal-overlay" @click.self="closeApprovalModal">
                    <div class="modal-box">
                        ...
                        <h4 class="modal-title">
                            {{ approvalModalType === 'approve' ? 'Aprobar Producto' : 'Rechazar Producto' }}
                        </h4>
                        <p class="modal-subtitle">
                            {{ approvalModalType === 'approve' 
                                ? 'Este producto será visible públicamente.' 
                                : 'Este producto será rechazado y no será visible.' 
                            }}
                        </p>
...
const fetchPostDetails = async () => {
    try {
        const response = await api.get(`/api/marketplace/details/${postId.value}`);
        postData.value = response.data.post;
    } catch (error) {
        console.error('Error fetching marketplace post details:', error);
...
const fetchComments = async () => {
    ...
    try {
        const { data } = await api.get(`/api/marketplace/${postId.value}/comments`);
...
const submitComment = async () => {
    ...
    try {
        await api.post(`/api/marketplace/${postId.value}/comments`, {
...
const saveEdit = async () => {
    ...
    try {
        await api.put(`/api/marketplace/approval-comments/${editingComment.value.comment_id}`, {
...
const confirmDelete = async () => {
    ...
    editSubmitting.value = true;
    try {
        await api.delete(`/api/marketplace/approval-comments/${editingComment.value.comment_id}`);
        await fetchComments();
...
const confirmApproval = async () => {
    ...
    try {
        ...
        await api.put(`/api/marketplace/${postId.value}/approve`, {
            approved: isApproved,
        ...
        console.error('Error updating marketplace approval status:', e);
...
onMounted(async () => {
    try {
        ...
        console.error('Error loading admin marketplace detail:', error);
...
:deep(.marketplace-card) {
    max-width: 100% !important;
    width: 100% !important;
}

:deep(.marketplace-card .card-hero) {
    height: 480px;
}
